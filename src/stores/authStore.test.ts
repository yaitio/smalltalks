...
describe('authStore', () => {
  ...
  describe('checkAuth', () => {
    it('sets authenticated state when user is authenticated', async () => {
      const mockUser = { id: '1', user_email: 'test@example.com' };
    ...
    it('sets unauthenticated state when user is not authenticated', async () => {
      vi.mocked(authClientModule.authClient.getUser).mockResolvedValue(null);
    ...
    it('handles errors gracefully', async () => {
      vi.mocked(authClientModule.authClient.getUser).mockRejectedValue(
